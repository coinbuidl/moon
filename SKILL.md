...
6. For first-time setup and after OpenClaw upgrades, run `moon install` before `moon verify --strict`. `install` is responsible for provenance self-heal (`plugins.installs.moon.*`) and, on macOS with installed binary, launchd watcher auto-start wiring (Windows/Linux: no autostart wiring yet).
